...
fdtd['background material'] = "<Object defined dielectric>"
...
def adjustGaussRefract(src_angle_incidence, src_hgt_Si, src_hgt_polymer, n_Si, n_polymer):
    x_offset = src_hgt_Si * np.tan(src_angle_incidence*np.pi/180)
    sin_th_prime = n_polymer/n_Si * np.sin(src_angle_incidence*np.pi/180)
    x_offset = x_offset + src_hgt_polymer*sin_th_prime/np.sqrt(1-sin_th_prime**2)
...
for xy_idx in range(0, 2):
    ...
    forward_src['divergence angle'] = 0.5*(lambda_min_um+lambda_max_um)*1e-6/(np.pi*src_beam_rad*1e-6)*(180/np.pi)
